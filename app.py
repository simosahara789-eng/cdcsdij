...
def download_images(listing: Dict[str, Any]) -> List[str]:
    ...
    for i, photo in enumerate(photos):
        status_text.text(f"Downloading image {i1} of {len(photos)}")
        image_url = None
        ...
        try:
            ...
            if not image_url:
                st.warning(f"Could not find image URL for image {i1}")
                continue
            ...
            if img_response.status_code == 200:
                ...
                st.warning(f"Failed to download image {i1}: HTTP {img_response.status_code}")
        except Exception as exc:  # noqa: BLE001
            st.warning(f"Error downloading image {i1}: {exc}")
...
def upload_images(api_key: str, listing_id: str, image_paths: List[str]) -> bool:
    ...
    for i, image_path in enumerate(image_paths):
        status_text.text(f"Uploading image {i1} of {len(image_paths)}")
        ...
        for endpoint in endpoints:
            ...
            for field_name in ("photo", "image", "file"):
                ...
                if response.status_code in (200, 201, 202, 204):
                    st.write(f"✅ Uploaded image {i1} via {endpoint} ({field_name})")
        ...
        if not uploaded:
            st.error(f"❌ Failed to upload image {i1} after trying all endpoints.")
